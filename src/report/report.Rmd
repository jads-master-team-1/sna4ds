...
keywords: "European Parliament, MEP, cohesion, country, party"
wordcount: 4948
...
ind_clim_votes <- dplyr::select(mep,4:19)
soc_clim_votes <- dplyr::select(mep,20:63)
...
The results from these votes were scraped from the VoteWatch.eu website on 20th October 2021. VoteWatch is a non-profit organisation with the goal of providing insight into the European Parliament and their decision making [@noauthor_votewatchmethodology_nodate]. The code used to scrape the data can be found in the appendix see [A.1 Scraping code (VoteWatch)]. The sociologically focused bill has 45 roll-call votes while the more industrially focused bill has 16. The full official documents of the votes can be found in the appendix, see [H. Impacts of Climate Change Document] and [G. Coal & Steel Vote Document]. The appendix also contains some descriptive measures of the data, see [B. Data summary].

The edges to be created from this data were created using a Python script written for the purpose, this can also be found in the appendix, see [A.2 Create edges].
...
```{r echo=FALSE}
tab_statistic(u_net_soc_clim, "Statistics for full sociological climate network")
tab_statistic(u_net_ind_clim, "Statistics for full industrial climate network")
```
...
## B. Data summary
```{r echo=FALSE}
#Number of MEPs represented in data
print('Number of MEPs in dataset')
nrow(mep)
# Spread of members of each European party in dataset
print('Party membership')
table(mep$party)
# Spread of countries of each European party in dataset
print('National representation')
table(mep$country)
# Number of roll-call votes in sociologically related climate vote
print('Number of roll-call votes in sociologically related climate vote')
ncol(soc_clim_votes)
# Proportion of each vote in sociologically related climate vote
print('Proportion of recorded votes in sociologically focused climate vote')
table <- table(data.frame(x=unlist(soc_clim_votes)))
prop.table(table)
# Number of roll-call votes in industrially related climate vote
print('Number of roll-call votes in industrially related climate vote')
ncol(ind_clim_votes)
# Proportion of each vote in sociologically related industrial vote
print('Proportion of recorded votes in industrially focused climate vote')
table <- table(data.frame(x=unlist(ind_clim_votes)))
prop.table(table)
```

## C. Network analysis

### C.1 Threshold determination
### C.1.1 Sociological climate bill
```{r echo=FALSE}
# Social climate bill
print('Histogram of spread of agreement of MEPs')
...
print('Network statistics at different threshold levels')
par(mfrow = c(2, 2), cex = 0.6)
...
```{r echo=FALSE}
print('Visualiations of graphs at different threshold levels (isolates removed for clarity')
par(mfrow = c(1, 2))
...
### C.1.3 Percentage Threshold determination
...
```{r echo=FALSE}
print('Networks at different thresholds for the sociologically focused bill')
par(mfrow=c(1, 2))
...
tab_statistic(uw_net_soc_clim_100, "Statistics for social climate network with threshold 100%")
```

```{r echo=FALSE}
print('Networks at different thresholds for the industrially focused bill')
par(mfrow=c(1, 2))
