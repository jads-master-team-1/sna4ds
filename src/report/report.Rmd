...
edges_climate <- read.csv("../../data/edges_for_against_climate.csv", header = T, sep = ",")
...
Another interesting political clash to investigate is that between the interests from certain national parties with those from the European group. For instance, the European Christen Democrats group (EPP) has been in a big dispute with Hungary's Fidesz party, eventually even leading to the national party leaving the group after being threatened to get kicked out [@de_la_baume_orbans_2021]. 
...
The data used for this project comes from an open source provided by EU. Lots of studies are conducted such as [@cherepnalkoski_karpf_mozetic_grcar_2016]. The mentioned studies are examples of how Twitter statuses of members of the European Parliament (MEP) can be connected to predict voting. Coalitions and political party cohesion are strong indicators of which vote will be final for the given MEP. The European Parliament is constructed in such a way where one political coalition may not have the overall majority to vote in or out a resolution. It is made on purpose to encourage discussion, debate and legal amendments. What tips the voting in favour or against are the members that vote against what is “expected” from them, in the sense of against their priors. Having an idea of when or how these events happen are interesting to investigate. The current literature has a similar methodology and investigative scope as our project. Our paper will corroborate or not with their findings. Towards the end it will be explained why it is more nuanced.
Our work is going to contribute to the field by providing additional insight into the voting behaviors of given MEPs. Our data being recent (2021) can suggest whether voting patterns are staying similar in time in comparison to other studies such as [@A_network_science_approach_to_social_cohesion_in_european_societies_2021-2026; @Political_network_size_and_its_antecedents_and_consequences_2013; @Political_networks_a_structured_bibliography_2007; @the_oxford_handbook_of_political_networks_2017]. 

The study will give possible factors that influence the final voting outcome for a given bill. The tools deployed to tackle this problem are described down below. They will allow to compare objectively using statistical tools such as ERGM. The creation of these models allow a better understanding of which factors influence the formation and dissociation of coalitions and political party cohesions.
...
Quadratic assignment procedure (QAP) is another method that will be used for this project. It scrambles the dependent variable data through several permutations. By taking the data, and “scrambling” it repeatedly, resulting in multiple random datasets with the dependent variable—and then multiple analyses can be performed. It is similar to a bootstrapping method but applied in the field of network analysis.

When using the ERGM method, the networks cannot be too dense. In this research, edges are drawn when people agree often enough based on an arbitrary threshold. Agreeing is defined as both MEPs saying yes or no to a bill. When taking all votes across one or two bills, a network can be made with weights being the number of times these two MEPs agree on an amendment or legal portion of the overarching document.
<!--
Where should the above paragraph be placed? Perhaps it should be moved to methodology?
-->
...
```{r echo=FALSE}
edges_climate$AgreementPercent <- round(edges_climate$Agreement/44, digits=2)
edges_industry$AgreementPercent <- round(edges_industry$Agreement/16, digits=2)
```

```{r echo=FALSE}
w_edges_climate <- edges_climate[edges_climate[, "Agreement"]>=1, c("MEP_1","MEP_2")]
w_network_climate <- igraph::graph_from_data_frame(w_edges_climate, vertices=mep, directed=FALSE)
w_network_climate  <- igraph::set_edge_attr(w_network_climate , "weight", value= edges_climate$AgreementPercent)

cn <- c("Measurement", "Value")
nonodes <- c("Number of nodes",igraph::vcount(w_network_climate))
noedges <- c("Number of edges",igraph::ecount(w_network_climate))
trans <- c("Transitivity", sprintf(igraph::transitivity(w_network_climate), fmt = '%#.3f'))
dens <- c("Graph density", sprintf(igraph::edge_density(w_network_climate), fmt = '%#.3f'))
avg_path <- c("Average path length", sprintf(igraph::mean_distance(w_network_climate), fmt = '%#.3f'))
diam <- c("Diameter", igraph::diameter(w_network_climate))
tab <- rbind(cn, nonodes,noedges,trans, dens, avg_path, diam)
rownames(tab) <- NULL
knitr::kable(tab, caption = "Statistics for weighted climate network")

w_edges_industry <- edges_industry[edges_industry[, "Agreement"]>=1, c("MEP_1","MEP_2")]
w_network_industry <- igraph::graph_from_data_frame(w_edges_industry, vertices=mep, directed=FALSE)
w_network_industry  <- igraph::set_edge_attr(w_network_industry , "weight", value= edges_industry$AgreementPercent)

cn <- c("Measurement", "Value")
nonodes <- c("Number of nodes",igraph::vcount(w_network_industry))
noedges <- c("Number of edges",igraph::ecount(w_network_industry))
trans <- c("Transitivity", sprintf(igraph::transitivity(w_network_industry), fmt = '%#.3f'))
dens <- c("Graph density", sprintf(igraph::edge_density(w_network_industry), fmt = '%#.3f'))
avg_path <- c("Average path length", sprintf(igraph::mean_distance(w_network_industry), fmt = '%#.3f'))
diam <- c("Diameter", igraph::diameter(w_network_industry))
tab <- rbind(cn, nonodes,noedges,trans, dens, avg_path, diam)
rownames(tab) <- NULL
...
plot_statistic(igraph::transitivity, edges_climate, 44, "Global transitivity", "Transitivity as a function of agreement threshold")
plot_statistic(igraph::edge_density, edges_climate, 44, "Network density", "Network density as a function of agreement threshold")
plot_statistic(igraph::mean_distance, edges_climate, 44, "Average path length", "Average path length a function of agreement threshold")
plot_statistic(igraph::diameter, edges_climate, 44, "Diameter", "Diameter as a function of agreement threshold")
mtext("Climate Motion",side=3,line=-19,outer=TRUE)


#Industry
hist(edges_industry$Agreement, col = "lightblue", xlab = "Agreement",main='Histogram of spread of agreement between MEPs')
par(mfrow = c(2,2), cex=0.6)
plot_statistic(igraph::transitivity, edges_industry, 16, "Global transitivity", "Transitivity as a function of agreement threshold")
plot_statistic(igraph::edge_density, edges_industry, 16, "Network density", "Network density as a function of agreement threshold")
plot_statistic(igraph::mean_distance, edges_industry, 16, "Average path length", "Average path length a function of agreement threshold")
plot_statistic(igraph::diameter, edges_industry, 16, "Diameter", "Diameter as a function of agreement threshold")
...
par(mfrow=c(1,2))
plot_network(edges_climate,12,igraph::layout_with_lgl)
plot_network(edges_climate,17,igraph::layout_with_lgl)

par(mfrow=c(1,2))
plot_network(edges_climate,38,igraph::layout_with_graphopt)
plot_network(edges_climate,44,igraph::layout_with_graphopt)
```
...
par(mfrow=c(1,2))
plot_network(edges_industry,3,igraph::layout_with_lgl)
plot_network(edges_industry,5,igraph::layout_with_lgl)

par(mfrow=c(1,2))
plot_network(edges_industry,11,igraph::layout_with_graphopt)
plot_network(edges_industry,14,igraph::layout_with_graphopt)
```
...
```{r echo=FALSE}
p1 <- hist(edges_industry$AgreementPercent)
p2 <- hist(edges_climate$AgreementPercent)
...
#Plot network
par(mfrow=c(1,2))
network_climate_35perc <- plot_network_perc(edges_climate,0.35,igraph::layout_with_graphopt)
hist(igraph::degree(network_climate_35perc), ylim=c(0,600), xlab="Degree", main='Degree distribution with threshold 35%')

par(mfrow=c(1,2))
network_climate_95perc <- plot_network_perc(edges_climate,0.95,igraph::layout_with_graphopt)
hist(igraph::degree(network_climate_95perc), ylim=c(0,600), xlab="Degree", main='Degree distribution with threshold 90%')
```
```{r echo=FALSE}
cn <- c("Measurement", "Value")
nonodes <- c("Number of nodes",igraph::vcount(network_climate_35perc))
noedges <- c("Number of edges",igraph::ecount(network_climate_35perc))
trans <- c("Transitivity", sprintf(igraph::transitivity(network_climate_35perc), fmt = '%#.3f'))
dens <- c("Graph density", sprintf(igraph::edge_density(network_climate_35perc), fmt = '%#.3f'))
avg_path <- c("Average path length", sprintf(igraph::mean_distance(network_climate_35perc), fmt = '%#.3f'))
diam <- c("Diameter", igraph::diameter(network_climate_35perc))
tab <- rbind(cn, nonodes,noedges,trans, dens, avg_path, diam)
rownames(tab) <- NULL
knitr::kable(tab, caption = "Statistics for climate network with threshold 35%")

cn <- c("Measurement", "Value")
nonodes <- c("Number of nodes",igraph::vcount(network_climate_95perc))
noedges <- c("Number of edges",igraph::ecount(network_climate_95perc))
trans <- c("Transitivity", sprintf(igraph::transitivity(network_climate_95perc), fmt = '%#.3f'))
dens <- c("Graph density", sprintf(igraph::edge_density(network_climate_95perc), fmt = '%#.3f'))
avg_path <- c("Average path length", sprintf(igraph::mean_distance(network_climate_95perc), fmt = '%#.3f'))
diam <- c("Diameter", igraph::diameter(network_climate_95perc))
tab <- rbind(cn, nonodes,noedges,trans, dens, avg_path, diam)
rownames(tab) <- NULL
...
#Plot network
par(mfrow=c(1,2))
network_industry_35perc <- plot_network_perc(edges_industry,0.35,igraph::layout_with_lgl)
hist(igraph::degree(network_industry_35perc), ylim=c(0,600), xlab="Degree", main='Degree distribution with threshold 35%')

par(mfrow=c(1,2))
network_industry_95perc <- plot_network_perc(edges_industry,0.95,igraph::layout_with_graphopt)
hist(igraph::degree(network_industry_95perc), ylim=c(0,600), xlab="Degree", main='Degree distribution with threshold 95%')
```
```{r echo=FALSE}
cn <- c("Measurement", "Value")
nonodes <- c("Number of nodes",igraph::vcount(network_industry_35perc))
noedges <- c("Number of edges",igraph::ecount(network_industry_35perc))
trans <- c("Transitivity", sprintf(igraph::transitivity(network_industry_35perc), fmt = '%#.3f'))
dens <- c("Graph density", sprintf(igraph::edge_density(network_industry_35perc), fmt = '%#.3f'))
avg_path <- c("Average path length", sprintf(igraph::mean_distance(network_industry_35perc), fmt = '%#.3f'))
diam <- c("Diameter", igraph::diameter(network_industry_35perc))
tab <- rbind(cn, nonodes,noedges,trans, dens, avg_path, diam)
rownames(tab) <- NULL
knitr::kable(tab, caption = "Statistics for industry network with threshold 35%")

cn <- c("Measurement", "Value")
nonodes <- c("Number of nodes",igraph::vcount(network_industry_95perc))
noedges <- c("Number of edges",igraph::ecount(network_industry_95perc))
trans <- c("Transitivity", sprintf(igraph::transitivity(network_industry_95perc), fmt = '%#.3f'))
dens <- c("Graph density", sprintf(igraph::edge_density(network_industry_95perc), fmt = '%#.3f'))
avg_path <- c("Average path length", sprintf(igraph::mean_distance(network_industry_95perc), fmt = '%#.3f'))
diam <- c("Diameter", igraph::diameter(network_industry_95perc))
tab <- rbind(cn, nonodes,noedges,trans, dens, avg_path, diam)
rownames(tab) <- NULL
...
w_network_climate_nw <- intergraph::asNetwork(w_network_climate)
w_network_industry_nw <- intergraph::asNetwork(w_network_industry)
network_climate_35perc_nw <- intergraph::asNetwork(network_climate_35perc)
network_climate_95perc_nw <- intergraph::asNetwork(network_climate_95perc)
network_industry_35perc_nw <- intergraph::asNetwork(network_industry_35perc)
network_industry_95perc_nw <- intergraph::asNetwork(network_industry_95perc)
```

### QAP test
In this section the correlation between the the climate vote network and the industry vote network was analysed with QAP-tests to give an initial indication of the answer to the first research question. The correlation between these networks can be considered in a number of ways, including inserting them in ERGMs as a predictor for the other respective networks. This will be carried out later since this can provide added information about the correlation while controlling for the other network characteristics. However, to already get a view on the correlation here, in a faster manner, QAP tests were carried out. A QAP test can be used here as both networks cover the same group of MEPs, therefore the linear subspace method can be used.Calculating the correlation here therefore involves comparing the existence of an edge between two MEPs in the first network with the existence of an edge between the same two MEPs in the second network.

A QAP test involves permuting both networks multiple times and each time calculating the correlation of the two networks to create a distribution of correlations.This is then compared to the correlations of the original networks to see if the statistic that is calculated on the original graphs is extreme for a graph controlled by this size and structure. This likeliness of observing this statistic is reflected in the p-value returned.This therefore uses the original graph and creates a distribution based for comparison of what the association would be under the null hypothesis of there being no association.

QAP tests were carried out on the full network, the 35% threshold network and the 95% threshold network for full coverage and also to see if there is different association at different level of agreement. For example, it's possible that there could be less correlation with lower thresholds due to random noise that could come from a chance agreement between two MEPs. In contrast, in the the comparison of the two 95% networks we expect an extremely high level of correlation as the agreement is not then likely by chance. and it is likely that if two MEPs agree on climate that they might have similar views about the importance of researching less environmentally impactful ways of producing steel.
First of all the basic correlation statistics for the three comparisons are calculated. The product-moment correlation statistic is used here as the most common statistic for correlation in networks [@MEGHANATHAN2021339]:

```{r echo=FALSE}
paste('Correlation of full networks (without weighting):', sprintf(sna::gcor(w_network_climate_nw,w_network_industry_nw,mode='graph'), fmt = '%#.3f'), sep=" ")
paste('Correlation of 35% threshold network:', sprintf(sna::gcor(network_climate_35perc_nw,network_industry_35perc_nw,mode='graph'), fmt = '%#.3f'), sep=" ")
paste('Correlation of 95% threshold network:', sprintf(sna::gcor(network_climate_95perc_nw,network_industry_95perc_nw,mode='graph'), fmt = '%#.3f'),sep=" ")
c_m <- network::as.sociomatrix(w_network_climate_nw,matrix.type='adjacency',attrname='weight')
i_m <- network::as.sociomatrix(w_network_industry_nw,matrix.type='adjacency',attrname='weight')
paste('Correlation of full weighted networks:', sprintf(sna::gcor(c_m, i_m,mode='graph'), fmt = '%#.3f'), sep=" ")
```

Interestingly, in contrast to the prediction, the correlation from the initial graph is significantly higher than the correlation between the two thresholded networks. However, upon examination this is likely due to the very high number of edges in the initial network- if all MEPs agree with the other MEPs on at least one vote then the whole network will be connected. This is reflected in the high densities of these graphs (0.964 and 0.977). This would explain the very high level of association as the probability is therefore very high that there will be a matching edge in the other network. The result from the thresholded networks is perhaps then more interesting. The correlation of the weighted graph is very low. However, nothing about the correlation of these graphs can be concluded without knowing how significant each association result is compared with the result from the null hypothesis. The QAP test is therefore carried out for this.

```{r echo=FALSE}
mepCor <- sna::qaptest(list(w_network_climate_nw, w_network_industry_nw), 
...
```{r echo=FALSE}
mepCor35 <- sna::qaptest(list(network_climate_35perc_nw, network_industry_35perc_nw), 
                       FUN = sna::gcor, g1 = 1, g2 = 2,mode='graph',reps = 1000)
...
```
```{r echo=FALSE}
mepCor95 <- sna::qaptest(list(network_climate_95perc_nw, network_industry_95perc_nw), 
                       FUN = sna::gcor, g1 = 1, g2 = 2, mode='graph', reps = 1000)
...
```
```{r echo=FALSE}
mepCor <- sna::qaptest(list(c_m, i_m), 
                       FUN = sna::gcor, g1 = 1, g2 = 2,mode='graph', reps = 1000)
sum_mepCor <- summary(mepCor)
sum_mepCor
sna::plot.qaptest(mepCor, xlim=c(min(sum_mepCor[[2]])-0.005,sum_mepCor[[1]]))
...
From the results it can be seen that all of these correlation results are very significantly significant. For all of these graphs based on the p-values there would be a 0.2% chance or less of observing these correlations based on chance alone.The significance of each correlation result is also put into scale in the diagram. While the full graph had the highest correlation statistic, the statistic is only around three times greater than the correlation from the created simulated graphs. In contrast, the correlation of the 95% networks is over 12 times larger than the highest correlation of the simulated graphs. This perhaps indicates that there is more association between actors with lots of agreement in both networks.
The most interesting graph is perhaps the QAP results from the weighted networks. While the association between the graphs is still significant, there is an overlap between the result observed in the networks and the permuted graphs in the QAP test. 
From the results it can therefore be concluded that there is an association between how pairs of MEPs agreed in the climate motion and industry motion for both weighted agreement, and with thresholds for incidental agreement and almost total agreement.This therefore provides evidence in line with the first hypothesis of this paper.
