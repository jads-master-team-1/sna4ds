...
In order to be able to compare the analysis of both networks it was considered desirable to use the same thresholds on both. Therefore percentage thresholds were identified to be used: above 40% agreement and above 95% agreement. These both relate to the two ranges identified in the descriptive models above and their respective medians.
...
#Plot network
par(mfrow=c(1,2))
network_climate_40perc <- plot_network_perc(edges_climate,0.4,igraph::layout_with_graphopt)
hist(igraph::degree(network_climate_40perc), ylim=c(0,600), xlab="Degree", main='Degree distribution with threshold 40%')
...
```{r echo=FALSE}
cn <- c("Measurement", "Value")
nonodes <- c("Number of nodes",igraph::vcount(network_climate_40perc))
noedges <- c("Number of edges",igraph::ecount(network_climate_40perc))
trans <- c("Transitivity", sprintf(igraph::transitivity(network_climate_40perc), fmt = '%#.3f'))
dens <- c("Graph density", sprintf(igraph::edge_density(network_climate_40perc), fmt = '%#.3f'))
avg_path <- c("Average path length", sprintf(igraph::mean_distance(network_climate_40perc), fmt = '%#.3f'))
diam <- c("Diameter", igraph::diameter(network_climate_40perc))
tab <- rbind(cn, nonodes,noedges,trans, dens, avg_path, diam)
rownames(tab) <- NULL
...
#Plot network
par(mfrow=c(1,2))
network_industry_40perc <- plot_network_perc(edges_industry,0.4,igraph::layout_with_lgl)
hist(igraph::degree(network_industry_40perc), ylim=c(0,600), xlab="Degree", main='Degree distribution with threshold 40%')

par(mfrow=c(1,2))
network_industry_95perc <- plot_network_perc(edges_industry,0.95,igraph::layout_with_lgl)
hist(igraph::degree(network_industry_95perc), ylim=c(0,600), xlab="Degree", main='Degree distribution with threshold 95%')
```
```{r echo=FALSE}
cn <- c("Measurement", "Value")
nonodes <- c("Number of nodes",igraph::vcount(network_industry_40perc))
noedges <- c("Number of edges",igraph::ecount(network_industry_40perc))
trans <- c("Transitivity", sprintf(igraph::transitivity(network_industry_40perc), fmt = '%#.3f'))
dens <- c("Graph density", sprintf(igraph::edge_density(network_industry_40perc), fmt = '%#.3f'))
avg_path <- c("Average path length", sprintf(igraph::mean_distance(network_industry_40perc), fmt = '%#.3f'))
diam <- c("Diameter", igraph::diameter(network_industry_40perc))
tab <- rbind(cn, nonodes,noedges,trans, dens, avg_path, diam)
rownames(tab) <- NULL
