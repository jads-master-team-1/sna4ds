...
_RQ1: “What is the cohesion of agreement, based on the climate and industry bill, within every European group?”_

Along with the research question, the following hypothesis is formulated:

_H1: “We expect a high level of cohesion for both bills”._
...
### QAP test
In this section the correlation between the the climate vote network and the industry vote network was analysed with QAP-tests to give an initial indication to the first research question. The correlation between these networks can be considered in a number of ways, including inserting them in ERGMs as a predictor for the other respective networks. This will be carried out later since this can provide added information about the correlation while controlling for the other network characteristics. However, to already get a view on the correlation here, QAP tests were carried out. A QAP test can be used here as both networks cover the same group of MEPs, therefore the linear subspace method can be used.

A QAP test involves permutating both networks multiple times and each time calculating the correlation of the two networks to create a distribution of correlations.This is then compared to the correlations of the original networks to see if the statistic that is calculated on the original graphs is extreme. This is reflected in the p-value of the likelihood of this statistic being observed.

DEFINE ASSOCIATION SOMEWHERE
CHECK- WEIGHT NOT INCLUDED? IF SO- STATE SOMEWHERE

QAP tests were carried out on the full network, the 35% threshold network and the 95% threshold network for full coverage and also to see if there is different association at different level of agreement. For example, it's possible that there could be less correlation in the full network due to random noise that could come from a chance agreement between two MEPs. In contrast, in the the comparison of the two 95% networks we expect an extremely high level of correlation as the agreement is not then likely by chance and it is likely that if two MEPs agree on climate that they might have similar views about the coal and steel industry (add source here!!!).

```{r echo=FALSE}
paste('Correlation of full networks:', sprintf(sna::gcor(w_network_climate_nw,w_network_industry_nw), fmt = '%#.3f'), sep=" ")
paste('Correlation of 35% threshold network:', sprintf(sna::gcor(network_climate_35perc_nw,network_industry_35perc_nw), fmt = '%#.3f'), sep=" ")
paste('Correlation of 95% threshold network:', sprintf(sna::gcor(network_climate_95perc_nw,network_industry_95perc_nw), fmt = '%#.3f'),sep=" ")
```

Interestingly, in contrast to the prediction, the correlation from the initial graphs is significantly higher than the correlation between the two thresholded networks. However, this may be due to the very high number of edges in the initial network- if nearly MEPs have one moment of agreement then nearly the whole network will be connected. This is reflected in the high densities of these graphs (0.964 and 0.977). This would explain the very high level of association as the probability is therefore very high that there will be a matching edge in the other network. he result from the thresholded networks is perhaps then more interesting. Next, the QAP test, can help further by showing how significant each association result is.
...
What does change in correlations say? Correlation highest for people who agree the most? (Maybe check which parties are removed/ remain in the 95% threshold one- they're the ones that stick to their guns?)

What proportion of possible edges are in the first two graphs?
