...
```{r echo=FALSE}
tab_statistic(w_net_soc_clim, "Statistics for weighted climate network")
tab_statistic(w_net_ind_clim, "Statistics for weighted industry network")
```
...
mtext("Climate Motion", side = 3, line = -18, outer = TRUE)
par(mfrow = c(1, 1))
...
plot_statistic(igraph::diameter,
               ...
               "Diameter",
               "Diameter vs. Agreement Threshold")
mtext("Industry Motion", side = 3, line = -18, outer = TRUE)
par(mfrow = c(1, 1))
```
...
As previously discussed, in the previous descriptive analysis both datasets had two specific ranges where the structure of the network significantly changed. The decision was therefore made to set the threshold for the networks used in this paper just above these ranges. This was chosen as it was considered likely that the more frequent levels of agreement between MEP's were likely be chance- particularly at lower levels.
